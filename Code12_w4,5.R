...
num_folds <- 5
grid_size <- 8  # From your gbm_grid
...
annotator_age_features_dev <- dev_data_annotators %>%
  ...
  select(tweet, prop_age_18_22_dev)
...
annotator_edu_features_dev <- dev_data_annotators %>%
  ...
  select(tweet, prop_doctorate_dev)
...
# se nenhuma, zv_names será character(0)
...
# mude p-ex. cutoff <- 1  para ser mais agressivo
...
# TASK 5
############################
# 1. Load Task 5 annotator features
task5_train <- read.csv("feature_files/task_5_features_train.csv", stringsAsFactors = FALSE)
task5_dev   <- read.csv("feature_files/task_5_features_dev.csv", stringsAsFactors = FALSE)

train_data <- read_csv("data/EXIST2025_train.csv")
dev_data <- read_csv("data/EXIST2025_dev_labeled.csv")

train_data_with_task_5 <- train_data %>%
  left_join(task5_train, by = "annotator_id")

dev_data_with_task_5 <- dev_data %>%
  rename(annotator_id = annotator) %>%
  left_join(task5_dev, by = "annotator_id")
