...
test_data <- read_csv("data/EXIST_test_nolabel.csv")
...
evaluate_model <- function(model, X_dev, y_dev, model_name, prob_method = "prob", threshold = 0.5) {
  if (prob_method == "prob") {
  ...
  # Calculate binary predictions using input threshold
  pred_class <- ifelse(pred_prob > threshold, "yes", "no") %>% factor(levels = c("no", "yes"))
  ...
  roc_obj <- pROC::roc(response = y_dev_factor, 
                       predictor = pred_prob, 
                       levels = c("no", "yes"), 
                       direction = "<")
...
compute_f1_macro <- function(threshold, probs, true_labels, positive_class) {
  preds <- ifelse(probs >= threshold, positive_class, setdiff(levels(true_labels), positive_class))
  preds <- factor(preds, levels = levels(true_labels))
  
  cm <- caret::confusionMatrix(preds, true_labels, positive = positive_class)
  
  # F1 for positive class
  precision_pos <- cm$byClass["Pos Pred Value"]
  recall_pos <- cm$byClass["Sensitivity"]
  f1_pos <- 2 * (precision_pos * recall_pos) / (precision_pos + recall_pos)
  
  # F1 for negative class
  # Calculate using confusion matrix table:
  tn <- cm$table[1,1]
  fp <- cm$table[2,1]
  fn <- cm$table[1,2]
  tp <- cm$table[2,2]
...
f1_scores <- sapply(thresholds, compute_f1_macro, probs = probs_gbm, true_labels = y_dev, positive_class = levels(y_dev)[2])

best_thresh <- thresholds[which.max(f1_scores)]
cat("\nBest threshold for F1_macro on dev set:", best_thresh, "\n")
cat("Best F1_macro:", max(f1_scores), "\n")
preds_gbm_tuned <- ifelse(probs_gbm >= best_thresh, levels(y_dev)[2], levels(y_dev)[1])
preds_gbm_tuned <- factor(preds_gbm_tuned, levels = levels(y_dev))

conf_mat_tuned <- caret::confusionMatrix(preds_gbm_tuned, y_dev, positive = levels(y_dev)[2])
print(conf_mat_tuned)
results_gbm <- evaluate_model(gbm_cv_model, X_dev_scaled_df, y_dev, "GBM", prob_method = "prob",threshold = 0.43)
# --- Task 2 ---
...
conf_mat_tuned <- caret::confusionMatrix(preds_gbm_tuned, y_dev, positive = levels(y_dev)[2])
print(conf_mat_tuned)
results_gbm <- evaluate_model(gbm_cv_model, X_dev_scaled_df, y_dev, "GBM", prob_method = "prob",threshold = 0.38)
...
colnames(dev_data)[colnames(dev_data) == "annotator"] <- "annotator_id"
colnames(test_data)[colnames(test_data) == "annotator"] <- "annotator_id"

# Load corresponding annotator cluster files (with kmeans_cluster only)
clusters_train <- read_csv("feature_files/features_task_3_train.csv")
clusters_dev   <- read_csv("feature_files/features_task_3_dev.csv")

# Function to add kmeans cluster count features per tweet
# Step 1: Join cluster info to each annotator in train_data
train_with_clusters <- train_data %>%
  left_join(clusters_train %>% distinct(annotator_id, kmeans_cluster), by = "annotator_id")

# Step 2: Count annotators per tweet and cluster
cluster_counts <- train_with_clusters %>%
  group_by(tweet, kmeans_cluster) %>%
  summarise(n_annotators = n(), .groups = "drop")

# Step 3: Pivot wide so each cluster gets a column
cluster_counts_wide <- cluster_counts %>%
  pivot_wider(names_from = kmeans_cluster,
              values_from = n_annotators,
              names_prefix = "cluster_",
              values_fill = 0)

# Step 4: Join back to tweet-level data if needed
# For example, get one row per tweet with cluster counts
tweets_with_cluster_counts <- train_data %>%
  distinct(tweet) %>%
  left_join(cluster_counts_wide, by = "tweet")

train_grouped <- train_grouped %>%
  left_join(cluster_counts_wide, by = "tweet")

# Join cluster info per annotator to each tweet-annotator row in dev set
dev_with_clusters <- dev_data %>%
  left_join(clusters_dev %>% distinct(annotator_id, kmeans_cluster), by = "annotator_id")

# Count how many annotators of each cluster per tweet in dev set
dev_cluster_counts <- dev_with_clusters %>%
  group_by(tweet, kmeans_cluster) %>%
  summarise(n_annotators = n(), .groups = "drop")

# Pivot wide to have one row per tweet with counts per cluster in dev set
dev_cluster_counts_wide <- dev_cluster_counts %>%
  pivot_wider(names_from = kmeans_cluster,
              values_from = n_annotators,
              names_prefix = "cluster_",
              values_fill = 0)
...
# Join cluster counts to the grouped dev data
dev_grouped <- dev_grouped %>%
  left_join(dev_cluster_counts_wide, by = "tweet")
...
# Create modeling dataset
modeling_data <- train_grouped %>%
  dplyr::select(tweet, final_label, all_of(features_task_3)) %>%
  rename(sexist = final_label) %>%
  mutate(across(where(is.numeric), ~ifelse(is.na(.) | is.nan(.) | is.infinite(.), 0, .)))
...
# Prepare features and target for dev set
X_dev <- dev_modeling %>% 
  dplyr::select(all_of(features_task_3)) %>%
  mutate(across(everything(), ~ ifelse(is.na(.), 0, .))) # Replace NA values with 0
y_dev <- as.factor(dev_modeling$sexist)
...
conf_mat_tuned <- caret::confusionMatrix(preds_gbm_tuned, y_dev, positive = levels(y_dev)[2])
print(conf_mat_tuned)
results_gbm <- evaluate_model(gbm_cv_model, X_dev_scaled_df, y_dev, "GBM", prob_method = "prob",threshold = 0.42)
...
# Load Task 5 features
task5_train <- read_csv("feature_files/task_5_features_train.csv")
task5_dev   <- read_csv("feature_files/task_5_features_dev.csv")

# Ensure proper column names
colnames(task5_train)[colnames(task5_train) == "annotator"] <- "annotator_id"
colnames(task5_dev)[colnames(task5_dev) == "annotator"] <- "annotator_id"

# Load annotated tweets
train_data <- read_csv("data/EXIST2025_train.csv")
dev_data   <- read_csv("data/EXIST2025_dev_labeled.csv")
colnames(train_data)[colnames(train_data) == "annotator"] <- "annotator_id"
colnames(dev_data)[colnames(dev_data) == "annotator"] <- "annotator_id"

# Merge features with tweet-annotator data
train_annotated <- left_join(train_data, task5_train, by = "annotator_id")
dev_annotated   <- left_join(dev_data, task5_dev,   by = "annotator_id")

# Aggregation function: mean per tweet
task5_feature_names <- c(
  "predicted_sexist_rate", "demographic_knn_bias", "bias_confidence",
  "mean_label", "label_variance", "label_entropy"
)

# Aggregate features at tweet level
train_task5_agg <- train_annotated %>%
  group_by(tweet) %>%
  summarise(across(all_of(task5_feature_names), mean, na.rm = TRUE)) %>%
  rename_with(~ paste0("agg_", .x), all_of(task5_feature_names))

dev_task5_agg <- dev_annotated %>%
  group_by(tweet) %>%
  summarise(across(all_of(task5_feature_names), mean, na.rm = TRUE)) %>%
  rename_with(~ paste0("agg_", .x), all_of(task5_feature_names))

# Join with train_grouped and dev_grouped
train_grouped <- left_join(train_grouped, train_task5_agg, by = "tweet")
dev_grouped   <- left_join(dev_grouped, dev_task5_agg,   by = "tweet")

train_grouped <- train_grouped %>%
  select(-starts_with("cluster_"), -matches("^contains_"))
...
# Define list of new features (Task 5)
task5_cols <- c(
  "agg_predicted_sexist_rate", "agg_demographic_knn_bias", "agg_bias_confidence",
  "agg_mean_label", "agg_label_variance", "agg_label_entropy"
)
results_ablation <- data.frame()
set.seed(123)
# Loop over these to test individually
for (i in 0:length(task5_cols)) {
  if (i == 0) {
    current_features <- features  # your original features
    label <- "Baseline"
  } else {
    f <- task5_cols[i]
    current_features <- c(features, f)
  ...
  train_mod <- train_grouped %>%
    select(tweet, final_label, all_of(current_features)) %>%
    rename(sexist = final_label) %>%
    mutate(across(where(is.numeric), ~ ifelse(is.na(.) | is.nan(.) | is.infinite(.), 0, .)))
  ...
  nzv <- caret::nearZeroVar(select(train_bal, -sexist), saveMetrics = TRUE)
  nzv_names <- rownames(nzv[nzv$zeroVar == TRUE, ])
  if (length(nzv_names) > 0) {
    train_bal <- select(train_bal, -all_of(nzv_names))
    dev_mod   <- select(dev_mod,   -all_of(nzv_names))
  }
  
  # Scale
  X_train_raw <- select(train_bal, -sexist)
  y_train <- factor(train_bal$sexist, levels = c("no", "yes"))
  ...
  X_dev_raw <- dev_mod[, train_columns, drop = FALSE]
  if (length(center_v) != ncol(X_dev_raw) || length(scale_v) != ncol(X_dev_raw)) next
  
  X_dev <- as.data.frame(scale(X_dev_raw, center = center_v, scale = scale_v))
  y_dev <- factor(dev_mod$sexist, levels = c("no", "yes"))
  ...
  model <- tryCatch({
    ...
    cat("Model failed for", label, ":", e$message, "\n")
    NULL
  })
  ...
  if (!is.null(model)) {
    ...
    f1_binary[is.na(f1_binary)] <- 0
    precision_pos[is.na(precision_pos)] <- 0
    precision_neg[is.na(precision_neg)] <- 0
    sensitivity[is.na(sensitivity)] <- 0
    specificity[is.na(specificity)] <- 0
    
    precision_macro <- (precision_pos + precision_neg) / 2
    recall_macro <- (sensitivity + specificity) / 2
    f1_macro <- ifelse(precision_macro + recall_macro == 0, 0,
    ...
    results_ablation <- bind_rows(results_ablation, new_row)
  }
}

print(results_ablation)
#  With agg_predicted_sexist_rate achieve better AUC and the same F1_Macro and agg_mean_label achive better F1_Macro

cat("\nTraining GBM with combined Task 5 features (agg_predicted_sexist_rate + agg_mean_label)...\n")

set.seed(123)

# Feature set
combined_features <- c(features, "agg_predicted_sexist_rate", "agg_mean_label")

# Prepare training data
train_mod <- train_grouped %>%
  select(tweet, final_label, all_of(combined_features)) %>%
  rename(sexist = final_label) %>%
  mutate(across(where(is.numeric), ~ ifelse(is.na(.) | is.nan(.) | is.infinite(.), 0, .)))
...
# Scale
X_train_raw <- select(train_bal, -sexist)
y_train_final <- factor(train_bal$sexist, levels = c("no", "yes"))
mat_train <- scale(X_train_raw)
X_train_scaled_df <- as.data.frame(mat_train)

center_v <- attr(mat_train, "scaled:center")
scale_v <- attr(mat_train, "scaled:scale")

# Ensure dev has matching features
train_columns <- colnames(X_train_scaled_df)
X_dev_raw <- dev_mod[, train_columns, drop = FALSE]
X_dev_scaled_df <- as.data.frame(scale(X_dev_raw, center = center_v, scale = scale_v))
y_dev <- factor(dev_mod$sexist, levels = c("no", "yes"))

# GBM cross-validated model
gbm_cv_model_final <- train(
  x = X_train_scaled_df,
  y = y_train_final,
  method = "gbm",
  trControl = cv_control,
...
# Extract best params
gbm_best_params <- gbm_cv_model_final$bestTune
cat("\nGBM Best Parameters:\n")
print(gbm_best_params)

# Predict on dev set
probs_gbm <- predict(gbm_cv_model_final, newdata = X_dev_scaled_df, type = "prob")[, 2]
preds_gbm <- ifelse(probs_gbm > 0.5, levels(y_dev)[2], levels(y_dev)[1])
preds_gbm <- factor(preds_gbm, levels = levels(y_dev))
...
# Store results
cv_results_list[["GBM_Task5_Combined"]] <- gbm_cv_model_final$results

# Evaluate with custom function
results_gbm <- evaluate_model(
  gbm_cv_model_final,
  X_dev_scaled_df,
  y_dev,
  model_name = "GBM_Task5_Combined",
  prob_method = "prob"
)
thresholds <- seq(0.1, 0.9, by = 0.01)
f1_scores <- sapply(thresholds, compute_f1_macro, probs = probs_gbm, true_labels = y_dev, positive_class = levels(y_dev)[2])
...
conf_mat_tuned <- caret::confusionMatrix(preds_gbm_tuned, y_dev, positive = levels(y_dev)[2])
print(conf_mat_tuned)
results_gbm <- evaluate_model(gbm_cv_model_final, X_dev_scaled_df, y_dev, "GBM", prob_method = "prob",threshold = 0.51)


### Predict TEST DATA ######## Task 6

annotators_test <- read_csv("feature_files/task_5_features_test_nolabel.csv")
colnames(annotators_test)[colnames(annotators_test) == "annotator"] <- "annotator_id"

# === Merge annotator info ===
test_annotated <- left_join(test_data, annotators_test, by = "annotator_id")

# === Task 2 Features ===

# 1. Age 18–22 proportion
test_age_feature <- test_annotated %>%
  filter(!is.na(tweet) & !is.na(age_group)) %>%
  group_by(tweet) %>%
  summarise(
    total_annotators = n(),
    count_18_22 = sum(age_group == "18-22", na.rm = TRUE),
    prop_age_18_22 = ifelse(total_annotators > 0, count_18_22 / total_annotators, 0),
    .groups = "drop"
  )

# 2. Doctorate proportion
test_edu_feature <- test_annotated %>%
  filter(!is.na(tweet) & !is.na(study_level)) %>%
  group_by(tweet) %>%
  summarise(
    total_annotators_edu = n(),
    count_doctorate = sum(study_level == "Doctorate", na.rm = TRUE),
    prop_doctorate = ifelse(total_annotators_edu > 0, count_doctorate / total_annotators_edu, 0),
    .groups = "drop"
  )

# 3. Middle Eastern flag
test_ethnicity_feature <- test_annotated %>%
  filter(!is.na(tweet) & !is.na(ethnicity)) %>%
  group_by(tweet) %>%
  summarise(
    middle_eastern = as.integer(any(ethnicity == "Middle Eastern", na.rm = TRUE)),
...
# 4. Algeria proportion
test_country_feature <- test_annotated %>%
  filter(!is.na(tweet) & !is.na(country)) %>%
  group_by(tweet) %>%
  summarise(
    total_annotators_country = n(),
    count_algeria = sum(country == "Algeria", na.rm = TRUE),
    prop_algeria = ifelse(total_annotators_country > 0, count_algeria / total_annotators_country, 0),
    .groups = "drop"
  )

# === Task 5 Aggregated Features ===
task5_agg <- test_annotated %>%
  group_by(tweet) %>%
  summarise(
    agg_predicted_sexist_rate = mean(predicted_sexist_rate, na.rm = TRUE),
    agg_mean_label = mean(mean_label, na.rm = TRUE),
    .groups = "drop"
  )
...
test_grouped <- test_data %>%
  distinct(tweet) %>%
  left_join(test_age_feature, by = "tweet") %>%
  left_join(test_edu_feature, by = "tweet") %>%
  left_join(test_ethnicity_feature, by = "tweet") %>%
  left_join(test_country_feature, by = "tweet") %>%
  left_join(task5_agg, by = "tweet") %>%
  mutate(across(everything(), ~ ifelse(is.na(.) | is.nan(.) | is.infinite(.), 0, .)))

# === Feature Engineering ===
test_grouped <- important_words(test_grouped)
test_grouped <- coloc(test_grouped)
test_grouped$tweet_length <- nchar(test_grouped$tweet)
test_grouped$word_count <- sapply(strsplit(test_grouped$tweet, "\\s+"), length)
test_grouped <- cbind(test_grouped, linguistic_complexity(test_grouped$tweet))
test_grouped <- cbind(test_grouped, sent_seq(test_grouped))
test_grouped <- cbind(test_grouped, extract_emotions(test_grouped$tweet))
...
combined_features <- c(
  ...
  "tweet_length", "word_count",
  "middle_eastern",
  "agg_predicted_sexist_rate", "agg_mean_label"
)

# === Prepare model matrix ===
test_modeling <- test_grouped %>%
  select(tweet, all_of(combined_features)) %>%
  mutate(across(where(is.numeric), ~ ifelse(is.na(.) | is.nan(.) | is.infinite(.), 0, .)))

X_test_raw <- test_modeling %>% select(-tweet)
X_test_scaled <- scale(X_test_raw, center = center_v, scale = scale_v)
X_test_scaled_df <- as.data.frame(X_test_scaled)

# === Predict ===
test_probs <- predict(gbm_cv_model_final, newdata = X_test_scaled_df, type = "prob")[, "yes"]
test_preds <- ifelse(test_probs >= 0.51, "YES", "NO")  # final label format

# Merge back to original test_data with prediction 
predicted_labels_df <- data.frame(
  tweet = test_modeling$tweet,
  label = test_preds
)

final_submission <- test_data %>%
  left_join(predicted_labels_df, by = "tweet")

# Save to required file
write_csv(final_submission, "Group_L_results.csv")
